...
For example, you can specify ``--run_spec probes.promptinject`` to run only the PromptInject framework's methods.
You can also specify specific probes instead of a probe family such as ``--run_spec probes.lmrc.SlurUsage`` to probe a model for generating slurs based on the Language Model Risk Cards framework.
...
.. code-block:: console
    ...
    garak --target_type openai --target_name gpt-5-nano --run_spec probes.encoding
...
.. code-block:: console
    ...
    garak --target_type huggingface --target_name gpt2 --run_spec probes.dan.Dan_11_0
