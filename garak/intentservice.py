...
def _get_stubs_txt(intent_code: str) -> Set[Stub]:
    ...
    stub_glob = stub_dir.glob(f"{intent_code}*.txt", case_sensitive=False)
...
def _get_stubs_json(intent_code: str) -> Set[Stub]:
    ...
    stub_glob = stub_dir.glob(f"{intent_code}*.json", case_sensitive=False)
...
def _get_stubs_yaml(intent_code: str) -> Set[Stub]:
    ...
    stub_glob = stub_dir.glob(f"{intent_code}*.y*ml", case_sensitive=False)
...
def get_intent_stubs(intent_code: str, text_only=True, conv_only=False) -> Set[Stub]:
    ...
    if not _validate_intent_specifier(intent_code):  # data sanitation before fs access
